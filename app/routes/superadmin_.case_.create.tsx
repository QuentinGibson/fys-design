import { ActionArgs, LoaderArgs, redirect, unstable_composeUploadHandlers, unstable_createFileUploadHandler, unstable_createMemoryUploadHandler, unstable_parseMultipartFormData } from "@remix-run/node";
import { Form, Link, useLoaderData } from "@remix-run/react";
import invariant from "tiny-invariant";
import { createCase, getCaseBySlug, updateCaseBySlug } from "~/models/case.server";
...
export const action = async ({ request, params }: ActionArgs) => {
  ...
  const tags = { set: tagsraw.map((tag: Option) => ({ id: tag.value })) }

  const inputData = { name, slug, content, logo: logoUrl, image: imageUrl, tags }

  const [errors, caseData] = await createCase(inputData)
  console.log("Error from create")
  console.table(errors)
  console.log("--------------------------")
  console.log("Project from create")
...
export default function SuperAdminProjectSlugRoute() {
  ...
  const [currentDropdownValue, setCurrentDropdownValue] = useState<{ label: string, value: string }[]>([] as { label: string, value: string }[])
